...
def is_rotated(original, check):
    return check in original + original and original != check
...
input2  = input[input.apply(lambda x: x['String2'] in (x['String1'] + x['String1']) and x['String1'] != x['String2'], axis=1)].reset_index(drop=True)
