...
input['Camp No'] = pd.to_numeric(input['Camp No'], errors='coerce')
input['group'] = (input['Camp No'].notna().cumsum())
input['group_name'] = np.where(input['Camp No'] == input['group'], input['Vaccine'], np.nan)
input['group_name'] = input.groupby('group')['group_name'].ffill()
